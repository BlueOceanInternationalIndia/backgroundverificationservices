...
import { ProfR } from "../../models/ProfR_Model.js";
...
router.post('/', aTr_Validate, async (req, res) => {
    ...
    await ProfR.create(req.body).then(async (entry) => {
        //Updating Logs
        const resp = await Log.findOneAndUpdate({uid: req.body.uid}, {form8: {enabled: true, submitted: true}}, {new: true});
...
router.post('/:uid', aTr_Validate, async (req, res) => {
    ...
    try {
        const data = await ProfR.find({uid: uid});
...
router.delete('/:uid', aTr_Validate, async (req, res) => {
    ...
    try {
        const data = await ProfR.findOneAndDelete({_id: uid});
        return res.status(200).json({
